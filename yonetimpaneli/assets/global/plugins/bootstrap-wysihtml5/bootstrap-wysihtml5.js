!function($, wysi) {
    ...
    var tpl = {
        ...
        //       "<a id='myelfinder' class='btn default" + size + "'' title='" + locale.image.insert + "' tabindex='-1'><i class='fa fa-folder'></i></a>" +
        //     "</li>";
        // },
        "paste": function(locale, options) {
            var size = (options && options.size) ? ' btn-'+options.size : '';

            return "<li>" +
                  "<div class='bootstrap-wysihtml5-paste-text-modal modal fade'>" +
                  ($.fn.modalmanager ? "" : "<div class='modal-dialog  modal-lg'>") +
                  " <div class='modal-content'>" +
                    "<div class='modal-header'>" +
                      "<a class='close' data-dismiss='modal'>&times;</a>" +
                      "<h3>" + locale.paste.paste + "</h3>" +
                    "</div>" +
                    "<div class='modal-body'>" +
                      "<textarea class='bootstrap-wysihtml5-paste-html-text form-control ' rows='8'></textarea>" +
                    "</div>" +
                    "<div class='modal-footer'>" +
                      "<a href='#' class='btn default' data-dismiss='modal'>" + locale.paste.cancel + "</a>" +
                      "<a href='#' class='btn btn-primary' data-dismiss='modal'>" + locale.paste.insert + "</a>" +
                    "</div>" +
                  "</div>" +
                   ($.fn.modalmanager ? "" : "</div>") +
                  "</div>" +
                  "<a class='btn default" + size + "' data-wysihtml5-command='pasteText' title='" + locale.paste.paste + "' tabindex='-1'><i class='fa fa-clipboard'></i></a>" +
                "</li>";
        },
    ...
    Wysihtml5.prototype = {
        ...
        initPasteText: function(toolbar) {
          var self = this;
          var pasteTextModal = toolbar.find('.bootstrap-wysihtml5-paste-text-modal');
          var pasteTextarea = pasteTextModal.find('.bootstrap-wysihtml5-paste-html-text');
          var insertButton = pasteTextModal.find('a.btn-primary');
          var initialValue = pasteTextarea.val();
          var caretBookmark;

          var pasteText = function() {
              var pasteTextVal = pasteTextarea.val();
              pasteTextarea.val(initialValue);
              self.editor.currentView.element.focus();
              if (caretBookmark) {
                self.editor.composer.selection.setBookmark(caretBookmark);
                caretBookmark = null;
              }
              console.log(pasteTextVal);

              self.editor.composer.commands.exec('insertText', pasteTextVal);

          };
          var pressedEnter = false;

          pasteTextarea.keypress(function(e) {
              if(e.which == 13) {
                  pasteText();
          ...
          insertButton.click(pasteText);

          pasteTextModal.on('shown', function() {
              pasteTextarea.focus();
          });

          pasteTextModal.on('hide', function() {
              self.editor.currentView.element.focus();
          });

          toolbar.find('a[data-wysihtml5-command=pasteText]').click(function() {
              var activeButton = $(this).hasClass("wysihtml5-command-active");

              if (!activeButton) {
                  self.editor.currentView.element.focus(false);
                  caretBookmark = self.editor.composer.selection.getBookmark();
                  pasteTextModal.appendTo('body').modal('show');
                  Metronic.initUniform(); //initialize uniform checkboxes
                  pasteTextModal.on('click.dismiss.modal', '[data-dismiss="modal"]', function(e) {
                      e.stopPropagation();
                  });
                  return false;
              }
    ...
    var locale = $.fn.wysihtml5.locale = {
        en: {
            ...
            link: {
                insert: "Bağlantıyı Ekle",
                cancel: "İptal",
                target: "Yeni pencerede açılsın."
            },
            image: {
                insert: "Görsel Ekle",
                cancel: "İptal"
            ...
            paste: {
                insert: "Temizle ve Ekle",
                paste: "Temizle ve Yapıştır",
                cancel: "İptal"
            },
            html: {
                edit: "HTML düzenle"
