...
<div class="page-container">
    ...
    $gelenid = $_GET["id"];
    $row = $db->get_row("SELECT * from urunler WHERE id = {$gelenid} ");
    ?>
    <!-- END SIDEBAR -->
    ...
    <div class="page-content-wrapper">
        ...
        <div class="portlet-title">
            <div class="caption">
                ...
                <span class="caption-subject font-green-sharp bold uppercase">Ürün Düzenle Yönetimi </span>
                <span class="caption-helper">Ürün Düzenle Düzenleme</span>
            </div>
            ...
            <div class="actions btn-set">
                ...
                <button class="btn green-haze btn-circle" onclick="islemControllerDuzenle('urunler','Duzenle','<?=$gelenid?>','urunler_duzenle.php?id=<?=$gelenid?>');  return false"><i class="fa fa-check"></i> Kaydet</button>
            </div>
        </div>
...
<div class="form-group">
                  ...
                  <div class="input-icon right">
                    <i class="fa"></i>
                      <input type="text" class="form-control required" name="name" placeholder="Ürün Adı" value="<?=$row->name?>">
                  </div>
                  </div>
              </div>
<!--labelname-->
      ...
      <div class="form-group">
                      ...
                      <div class="col-md-10">
                          ...
                          <?php foreach ($db->get_results("SELECT * from kategoriler ORDER BY adi ASC ") as $value) {?>
                            <option value="<?=$value->id?>" <?php if ($row->katid == $value->id) { echo "selected";}?>><?=$value->adi?></option>
                          <?php }?>
                          </select>
                      </div>
                  </div>
...
<!--label{label_name}-->
      <div class="form-group">
                      ...
                      <div class="col-md-10">
                          ...
                          <option value="" >Bir Seçim Yapınız.</option>
                          <?php foreach ($db->get_results("SELECT * from a_sinif_enerji ORDER BY adi ASC ") as $value) {?>
                            <option value="<?=$value->id?>" <?php if ($row->sogutma == $value->id) { echo "selected";}?>><?=$value->adi?></option>
                          <?php }?>
                          </select>
                      </div>
                  </div>
...
<!--label{label_name}-->
      <div class="form-group">
                      ...
                      <div class="col-md-10">
                          ...
                          <option value="" >Bir Seçim Yapınız.</option>
                          <?php foreach ($db->get_results("SELECT * from a_sinif_enerji ORDER BY adi ASC ") as $value) {?>
                            <option value="<?=$value->id?>" <?php if ($row->isitma == $value->id) { echo "selected";}?>><?=$value->adi?></option>
                          <?php }?>
                          </select>
                      </div>
                  </div>
...
<div class="form-group">
                  ...
                  <div class="input-icon right">
                    <i class="fa"></i>
                      <input type="text" class="form-control required" name="sog_en_ver" placeholder="Soğutma Enerji Verimliliği" value="<?=$row->sog_en_ver?>">
                  </div>
                  </div>
              </div>
<!--labelsog_en_ver-->
...
<div class="form-group">
                  ...
                  <div class="input-icon right">
                    <i class="fa"></i>
                      <input type="text" class="form-control required" name="isitma_en_ver" placeholder="Isıtma Enerji Verimliliği" value="<?=$row->isitma_en_ver?>">
                  </div>
                  </div>
              </div>
<!--labelisitma_en_ver-->
...
<div class="form-group">
    ...
    <div class="col-md-10 gorsel_detay">
        ...
        <div class="fixed-dragger-cropperq" style="height: 400px"><img src="../../images/urunler/gorsel_<?=$row->id?>.jpg"></div>
...
<div class="form-actions right">
    ...
    <button class="btn green-haze btn-circle" onclick="islemControllerDuzenle('urunler','Duzenle','<?=$gelenid?>','urunler_duzenle.php?id=<?=$gelenid?>');"><i class="fa fa-check" ></i> Kaydet</button>
</div>
</div>
</div>
</form>
