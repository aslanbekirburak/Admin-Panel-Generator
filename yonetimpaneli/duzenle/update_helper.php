...
$adressname= $db->get_var("SELECT Php_adress FROM file_controllers WHERE file_id = '$adress_id'");
...
//default başlangıç yazma kısmı
  ...
  // direk update sayfasından alınacak $adress_id = $db->get_var("SELECT id FROM yetki_sayfalar WHERE sayfalar = '$adressname'");
  ...
  foreach ($result_form as $key => $value) {
    ...
    if($value->form_type=="text_input")
      {    //echo "foreach if input";
        ...
        text_input_writer($adressname,$value->label_name,$value->form_name,$value->form_selector_id,$value->placeholder);}

    if($value->form_type=="radio")
      {    //echo "foreach if radio";

        radio_writer($adressname,$value->label_name,$value->form_name,$value->label1,$value->label2,$value->value1,$value->value2);}

    if($value->form_type=="dropdown")
      {   // echo "foreach if dropdown";

        dropdown_writer($adressname,$value->label_name,$value->form_name,$value->form_selector_id,$value->Database_name,$value->Database_var_name);}

    // print_r($value->form_name ." ");
    // print_r(" - ".$value->label_name ." ");
  }

function radio_writer($adressname,$label_name,$form_name,$label1,$label2,$value1,$value2)
{
  //$adress_id=$_POST["page_id"];
  //$adressname= $db->get_var("SELECT Php_adress FROM file_controllers WHERE file_id = '$adress_id'");
...
function dropdown_writer($adressname,$label_name,$form_name,$form_selector_id,$Database_name,$Database_var_name)
{
  //$adress_id=$_POST["page_id"];
  //$adressname= $db->get_var("SELECT Php_adress FROM file_controllers WHERE file_id = '$adress_id'");
...
function text_input_writer($adressname,$label_name,$form_name,$form_selector_id,$placeholder)
{
  //$adress_id=$_POST["page_id"];
//  $adressname= $db->get_var("SELECT Php_adress FROM file_controllers WHERE file_id = '$adress_id'");
